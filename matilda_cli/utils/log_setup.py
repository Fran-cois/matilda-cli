...
def setup_loggers():
    ...
    file_handler_query_time = RotatingFileHandler(
        "logs/query_time.log", maxBytes=5 * 1024 * 1024, backupCount=5
    )
    ...
    file_handler_query_results = RotatingFileHandler(
        "logs/query_results.log", maxBytes=5 * 1024 * 1024, backupCount=5
    )
    formatter_query_results = logging.Formatter(
