...
def run_cmd(cmd_string: str, timeout: int = None, memory_limit_gb: float = 30) -> bool:
    ...
    try:
        ...
        if "|" in cmd_list:
            ...
            with subprocess.Popen(cmd_list1, stdout=subprocess.PIPE) as p1, subprocess.Popen(
                cmd_list2, stdin=p1.stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            ) as p2:
            ...
            if output_file_index < len(cmd_list):
                ...
                with open(output_file, "w") as f:
                    ...
                    if process.returncode == 0:
                        ...
                        print(
                            f"Command failed with return code {process.returncode}: {errors.decode()}"
                        )
                        return False
