...
class TripleConverter:
        ...
        for table_name in self._get_table_names():
            ...
            for row in rows:
                ...
                if any(pk not in row_dict or row_dict[pk] is None for pk in pk_columns):
                    self.logger.error(
                        f"Missing primary keys in table {table_name} for row {row_dict}."
                    )
                    continue
                ...
                for attribute, value in row_dict.items():
                    ...
                    if attribute in fk_columns:
                        ...
                        try:
                            ...
                            ref_subject = self._generate_rdf_id(
                                ref_table, ref_pk_columns, row_dict_fk
                            )
                            triples.append((subject, predicate, ref_subject))
                        except Exception as e:
                            self.logger.error(
                                f"Error processing foreign key for table {table_name}: {e}"
                            )
    ...
    def _generate_rdf_id(
        self, table: str, primary_keys: List[str], row_dict: Dict[str, Any]
    ) -> str:
        try:
            pk_values = "_".join(
