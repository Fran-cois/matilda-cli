...
class QueryUtility:
    ...
    def get_join_row_count(
        ...
        self.logger_query_results.info(f"Query: {str(query)}; Result: {result_sqlite}")
    ...
    def _construct_threshold_query(
        self, join_conditions, disjoint_semantics, distinct, count_over, threshold
    ):
        ...
        query = select((func.count() > threshold).label("count_exceeds_threshold")).select_from(
            join_base
        )
    ...
    def _construct_count_query(self, join_conditions, disjoint_semantics, distinct, count_over):
        # Construct the query and return it along with conditions
        query, primary_key_conditions, join_base = self._construct_query_base(
            join_conditions, disjoint_semantics, distinct, count_over
        )
    ...
    def _construct_query_base(self, join_conditions, disjoint_semantics, distinct, count_over):
        ...
        try:
            join_bases, aliases, used_aliases, table_occurrences = self._process_join_conditions(
                condition_groups, disjoint_semantics
            )
        except Exception as e:
        ...
        if disjoint_semantics:
            primary_key_conditions = self._construct_primary_key_conditions(
        ...
        query = self._construct_select_query(
            join_base, distinct, primary_key_conditions, count_over, aliases
        )
        return query, primary_key_conditions, join_base
    ...
    def _organize_join_conditions(self, join_conditions: List[Tuple[str, int, str, str, int, str]]):
        ...
        for condition in join_conditions:
            table_name1, occurrence1, attribute_name1, table_name2, occurrence2, attribute_name2 = (
                condition
            )
            ...
            if len(sorted_key) == 2:
                ...
                for tn1, o1, attr1, tn2, o2, attr2 in group:
                    ...
                    if attr1 in columns_alias1 and attr2 in columns_alias2:
                        partial_join_conditions.append(
                            alias1.columns[attr1] == alias2.columns[attr2]
                        )
                    elif attr2 in columns_alias1 and attr1 in columns_alias2:
                        partial_join_conditions.append(
                            alias1.columns[attr2] == alias2.columns[attr1]
                        )
                ...
                if partial_join_conditions:
                    ...
                    join_bases.append(
                        (
                            f"{table_name1}_{occurrence1}",
                            f"{table_name2}_{occurrence2}",
                            join_condition,
                        )
                    )
                ...
                if table_name1 not in self.metadata.tables:
                    self.logger_query_time.error(
                        f"Table '{table_name1}' does not exist; skipping condition."
                    )
                    continue
                alias1 = self._get_or_create_alias(aliases, table_name1, occurrence1)
                partial_join_conditions = []
                for tn1, o1, attr1, tn2, o2, attr2 in group:
                    ...
                    if attr1 in columns_alias1 and attr2 in columns_alias1:
                        partial_join_conditions.append(
                            alias1.columns[attr1] == alias1.columns[attr2]
                        )
                if partial_join_conditions:
                    join_condition = and_(*partial_join_conditions)
    ...
    def _construct_primary_key_conditions(
        self, table_occurrences: Dict[str, set], aliases: Dict[str, Any], used_aliases: set
    ):
    ...
    def _construct_select_query(
        ...
        if count_over and len(count_over) > 0:
            ...
            )  # for future version of sqlalchemy
        ...
        for table_name, table in self.metadata.tables.items():
            for fk in table.foreign_keys:
                ...
                if ref_table not in self.metadata.tables:
                    self.logger_query_time.error(
                        f"Referenced table '{ref_table}' does not exist for foreign key '{local_column}' in table '{table_name}'."
                    )
                    continue
                ...
                if reference_column not in self.metadata.tables[ref_table].columns:
                    self.logger_query_time.error(
                        f"Referenced column '{reference_column}' does not exist in table '{ref_table}' for foreign key '{local_column}' in table '{table_name}'."
                    )
                    continue
