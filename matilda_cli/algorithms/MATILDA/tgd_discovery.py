...
from matilda_cli.utils.rules import Predicate, TGDRule
    ...
    try:
        ...
        for table_occurrence1 in range(max_nb_occurrence):
            for table_occurrence2 in range(max_nb_occurrence):
                for attr1, attr2 in compatible_attributes:
                    if max_nb_occurrence_per_table_and_column and (
...
def calculate_accuracy(
    ...
    """
    Calculate accuracy by counting actual rule violations.
    Accuracy = (tuples in body that have corresponding tuples in head) / (total tuples in body)

    This directly measures rule satisfaction in the database.
    """
    try:
        # Get body table and attribute for simple single-predicate rules
        if len(body) != 1 or len(head) != 1:
            return 1.0  # Complex rules - fallback to 1.0

        body_occurrence = list(body)[0]
        head_occurrence = list(head)[0]

        # Find the attribute that connects body and head
        body_attr = None
        head_attr = None

        for jia in candidate_rule:
            attr1, attr2 = jia
            if (attr1.i, attr1.j) == body_occurrence:
                body_attr = mapper.indexed_attribute_to_attribute(attr1)
            if (attr2.i, attr2.j) == body_occurrence:
                body_attr = mapper.indexed_attribute_to_attribute(attr2)
            if (attr1.i, attr1.j) == head_occurrence:
                head_attr = mapper.indexed_attribute_to_attribute(attr1)
            if (attr2.i, attr2.j) == head_occurrence:
                head_attr = mapper.indexed_attribute_to_attribute(attr2)

        if not body_attr or not head_attr:
            return 1.0

        # Use raw SQL for reliability
        from sqlalchemy import text

        # Count total tuples in body
        query_total = text(f"SELECT COUNT(DISTINCT {body_attr.name}) FROM {body_attr.table}")
        with db_inspector.db_manager.engine.connect() as conn:
            result = conn.execute(query_total)
            total_body = result.fetchone()[0]

        if total_body == 0:
            return 1.0

        # Count body tuples that have corresponding head tuples
        query_satisfying = text(
            f"""
            SELECT COUNT(DISTINCT b.{body_attr.name})
            FROM {body_attr.table} b
            WHERE EXISTS (
                SELECT 1 FROM {head_attr.table} h 
                WHERE h.{head_attr.name} = b.{body_attr.name}
            )
        """
        )
        with db_inspector.db_manager.engine.connect() as conn:
            result = conn.execute(query_satisfying)
            satisfying = result.fetchone()[0]

        accuracy = satisfying / total_body if total_body > 0 else 1.0
        logging.info(
            f"Accuracy calculated: {accuracy:.3f} ({satisfying}/{total_body}) for {body_attr.table} -> {head_attr.table}"
        )
        return accuracy

    except Exception as e:
        logging.warning(f"Error calculating accuracy: {e}")
        return 1.0
...
def calculate_coverage(
    ...
    """
    Calculate coverage: proportion of head tuples that are covered by the rule.
    Coverage = (tuples in head that have corresponding tuples in body) / (total tuples in head)
    """
    try:
        if len(body) != 1 or len(head) != 1:
            return 1.0
        ...
        # Count total tuples in head
        query_total = text(f"SELECT COUNT(DISTINCT {head_attr.name}) FROM {head_attr.table}")
        with db_inspector.db_manager.engine.connect() as conn:
            result = conn.execute(query_total)
            total_head = result.fetchone()[0]

        if total_head == 0:
            return 1.0

        # Count head tuples that have corresponding body tuples
        query_covered = text(
            f"""
            SELECT COUNT(DISTINCT h.{head_attr.name})
            FROM {head_attr.table} h
            WHERE EXISTS (
                SELECT 1 FROM {body_attr.table} b 
                WHERE b.{body_attr.name} = h.{head_attr.name}
            )
        """
        )
        with db_inspector.db_manager.engine.connect() as conn:
            result = conn.execute(query_covered)
            covered = result.fetchone()[0]

        coverage = covered / total_head if total_head > 0 else 1.0
        logging.info(
            f"Coverage calculated: {coverage:.3f} ({covered}/{total_head}) for {body_attr.table} -> {head_attr.table}"
        )
        return coverage

    except Exception as e:
        logging.warning(f"Error calculating coverage: {e}")
        return 1.0
...
def calculate_support(
    ...
    NOW RETURNS ACCURACY instead of theoretical support.
    ...
    :return: The accuracy value as a float.
    """
    cr_chains = CandidateRuleChains(candidate_rule).cr_chains
    ...
    # Calculate accuracy instead of theoretical support
    accuracy = calculate_accuracy(candidate_rule, body, head, db_inspector, mapper)
    return accuracy
...
def calculate_confidence(
    ...
    # Calculate coverage instead of theoretical confidence
    coverage = calculate_coverage(candidate_rule, body, head, db_inspector, mapper)
    return coverage
